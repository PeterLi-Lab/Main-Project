...
class FinalMatchingAnalysis:
    """Final matching analysis with focus on core improvements"""
    ...
    def simple_nearest_neighbor_matching(self, df, feature_cols, n_matches=2000):
        """Simple nearest neighbor matching with limited matches"""
        print("Performing simple nearest neighbor matching...")
        ...
        # Limit sample size for better matching
        n_treatment = min(n_matches, len(treatment_df))
        n_control = min(n_matches, len(control_df))
        
        # Sample treatment and control
        treatment_sample = treatment_df.sample(n=n_treatment, random_state=42)
        control_sample = control_df.sample(n=n_control, random_state=42)
        
        treatment_sample_features = treatment_features[treatment_sample.index]
        control_sample_features = control_features[control_sample.index]
        
        # Fit nearest neighbor model
        nn_model = NearestNeighbors(n_neighbors=1, algorithm='auto')
        nn_model.fit(control_sample_features)
        
        # Find matches
        distances, indices = nn_model.kneighbors(treatment_sample_features)
        ...
        for i, (treatment_idx, neighbor_indices) in enumerate(zip(treatment_sample.index, indices)):
            neighbor_idx = neighbor_indices[0]
            if neighbor_idx not in used_control_indices:
                matched_treatment.append(treatment_sample.loc[treatment_idx])
                matched_control.append(control_sample.iloc[neighbor_idx])
    ...
    def create_simple_visualization(self, df, feature_cols, balance_metrics, method_name):
        """Create simple balance visualization"""
        print(f"Creating simple visualization for {method_name}...")
    ...
    def create_tsne_visualization(self, df, feature_cols, method_name):
        """Create t-SNE visualization with proper perplexity"""
        ...
        plt.title(f't-SNE: {method_name} Matching Results')
        plt.legend()
...
def final_matching_analysis():
    """Main function for final matching analysis"""
    print("=== Final Matching Analysis ===\n")
    ...
    # Initialize analyzer
    analyzer = FinalMatchingAnalysis()
    ...
    try:
        matched_df = analyzer.simple_nearest_neighbor_matching(df, feature_cols, n_matches=2000)
        ...
        analyzer.create_simple_visualization(matched_df, feature_cols, balance_metrics, 'nearest_neighbor')
        analyzer.create_tsne_visualization(matched_df, feature_cols, 'nearest_neighbor')
        
        # Perform uplift analysis
        uplift_results = analyzer.create_uplift_analysis(matched_df, 'nearest_neighbor')
        
        # Save matched dataset
        output_file = 'uplift_model_data_final_matched.csv'
        ...
        return matched_df, balance_metrics, uplift_results
