...
def calculate_ipw_weights(propensity_scores, treatment):
    """Calculate IPW weights with stabilization"""
    # Overall treatment probability
...
def calculate_ipw_uplift_at_k(y_true, uplift_scores, treatment, propensity_scores, k_percent=20):
    ...
    if len(treatment_top_k) > 0:
        ...
        # If no treatment samples in top k%, use overall treatment mean
    ...
    if len(control_top_k) > 0:
        ...
        # If no control samples in top k%, use overall control mean
...
def calculate_ipw_baseline_metrics(y_true, treatment, propensity_scores):
    ...
    # Random targeting baseline (using IPW weights)
    np.random.seed(42)
    random_treatment = np.random.permutation(treatment)
    random_treatment_indices = random_treatment == 1
    random_control_indices = random_treatment == 0
    
    random_treatment_mean = (y_true[random_treatment_indices] * ipw_weights[random_treatment_indices]).sum() / ipw_weights[random_treatment_indices].sum() if ipw_weights[random_treatment_indices].sum() > 0 else 0
    random_control_mean = (y_true[random_control_indices] * ipw_weights[random_control_indices]).sum() / ipw_weights[random_control_indices].sum() if ipw_weights[random_control_indices].sum() > 0 else 0
    
    random_uplift = random_treatment_mean - random_control_mean
    
    return random_uplift, all_users_uplift

def segment_users_by_uplift_sensitivity_ipw(X, y_true, uplift_scores, treatment, propensity_scores, n_segments=4):
    """Segment users based on uplift sensitivity with IPW correction"""
    ...
    user_features['propensity_scores'] = propensity_scores
    ...
    # Calculate IPW weights
    p_treatment = treatment.mean()
    user_features['ipw_weights'] = np.where(
        user_features['treatment'] == 1,
        p_treatment / user_features['propensity_scores'],
        (1 - p_treatment) / (1 - user_features['propensity_scores'])
    )
    
    # Calculate segment metrics with IPW correction
    segment_metrics = []
    for segment in user_features['segment'].unique():
        ...
        # IPW treatment mean
        if len(treatment_segment) > 0:
            treatment_mean = (treatment_segment['y_true'] * treatment_segment['ipw_weights']).sum() / treatment_segment['ipw_weights'].sum()
        else:
            treatment_mean = 0
        
        # IPW control mean
        if len(control_segment) > 0:
            control_mean = (control_segment['y_true'] * control_segment['ipw_weights']).sum() / control_segment['ipw_weights'].sum()
        else:
            control_mean = 0
        ...
        relative_uplift_pct = (segment_uplift / control_mean * 100) if control_mean > 0 else 0
        ...
        segment_metrics.append({
            ...
            'treatment_mean': treatment_mean,
            'control_mean': control_mean,
            'uplift_absolute': segment_uplift,
            'uplift_relative_pct': relative_uplift_pct,
            'treatment_count': len(treatment_segment),
            'control_count': len(control_segment)
        })
    
    return segment_metrics

def main():
    """IPW-corrected uplift evaluation to address treatment imbalance"""
    
    print("=== IPW-CORRECTED UPLIFT EVALUATION ===")
    ...
    propensity_scores_test = np.clip(propensity_scores_test, 1e-4, 1 - 1e-4)
    ...
    print("\n=== IPW-CORRECTED EVALUATION METRICS ===")
    ...
    random_uplift, all_users_uplift = calculate_ipw_baseline_metrics(y_outcome_test, y_treatment_test, propensity_scores_test)
    print(f"\nIPW Baseline metrics:")
    print(f"IPW Random targeting uplift: {random_uplift:.4f} ({random_uplift*100:.2f}%)")
    print(f"IPW All-users targeting uplift: {all_users_uplift:.4f} ({all_users_uplift*100:.2f}%)")
    
    # IPW-corrected segment differences
    print(f"\n=== IPW-CORRECTED SEGMENT ANALYSIS ===")
    segment_metrics = segment_users_by_uplift_sensitivity_ipw(X_test, y_outcome_test, uplift_scores, y_treatment_test, propensity_scores_test)
    
    for segment in segment_metrics:
        print(f"{segment['segment']} segment:")
        print(f"  Count: {segment['count']:,}")
        print(f"  IPW Treatment mean: {segment['treatment_mean']:.4f}")
        print(f"  IPW Control mean: {segment['control_mean']:.4f}")
        print(f"  IPW Uplift (absolute): {segment['uplift_absolute']:.4f} ({segment['uplift_absolute']*100:.2f}%)")
        print(f"  IPW Uplift (relative): {segment['uplift_relative_pct']:.2f}%")
        print(f"  Treatment count: {segment['treatment_count']}, Control count: {segment['control_count']}")
    
    # Find high and low uplift segments
    if len(segment_metrics) >= 2:
        high_uplift_segment = max(segment_metrics, key=lambda x: x['uplift_absolute'])
        low_uplift_segment = min(segment_metrics, key=lambda x: x['uplift_absolute'])
        
        print(f"\nIPW Segment differences:")
        print(f"High-uplift cohort ({high_uplift_segment['segment']}):")
        print(f"  IPW Treatment mean: {high_uplift_segment['treatment_mean']:.4f}")
        print(f"  IPW Control mean: {high_uplift_segment['control_mean']:.4f}")
        print(f"  IPW Uplift: {high_uplift_segment['uplift_absolute']:.4f} ({high_uplift_segment['uplift_absolute']*100:.2f}%)")
        
        print(f"Low-uplift cohort ({low_uplift_segment['segment']}):")
        print(f"  IPW Treatment mean: {low_uplift_segment['treatment_mean']:.4f}")
        print(f"  IPW Control mean: {low_uplift_segment['control_mean']:.4f}")
        print(f"  IPW Uplift: {low_uplift_segment['uplift_absolute']:.4f} ({low_uplift_segment['uplift_absolute']*100:.2f}%)")
        
        print(f"Difference: {high_uplift_segment['uplift_absolute'] - low_uplift_segment['uplift_absolute']:.4f}")
    ...
    results = {
        'metric': [
            ...
            'IPW_Random_targeting_uplift', 'IPW_All_users_targeting_uplift',
    ...
    results_df.to_csv('ipw_corrected_uplift_evaluation_metrics.csv', index=False)
    
    # Export segment data
    segment_df = pd.DataFrame(segment_metrics)
    segment_df.to_csv('ipw_corrected_uplift_segment_analysis.csv', index=False)
    
    # Create summary report
    with open('ipw_corrected_uplift_evaluation_report.txt', 'w', encoding='utf-8') as f:
        f.write("=== IPW-CORRECTED UPLIFT EVALUATION REPORT ===\n\n")
        ...
        f.write(f"IPW Random targeting uplift: {random_uplift:.4f} ({random_uplift*100:.2f}%)\n")
        f.write(f"IPW All-users targeting uplift: {all_users_uplift:.4f} ({all_users_uplift*100:.2f}%)\n\n")
        
        f.write("=== IPW SEGMENT ANALYSIS ===\n")
        for segment in segment_metrics:
            f.write(f"{segment['segment']} segment:\n")
            f.write(f"  Count: {segment['count']:,}\n")
            f.write(f"  IPW Treatment mean: {segment['treatment_mean']:.4f}\n")
            f.write(f"  IPW Control mean: {segment['control_mean']:.4f}\n")
            f.write(f"  IPW Uplift (absolute): {segment['uplift_absolute']:.4f} ({segment['uplift_absolute']*100:.2f}%)\n")
            f.write(f"  IPW Uplift (relative): {segment['uplift_relative_pct']:.2f}%\n\n")
    
    print("Results exported to:")
    print("- ipw_corrected_uplift_evaluation_metrics.csv")
    print("- ipw_corrected_uplift_segment_analysis.csv")
    print("- ipw_corrected_uplift_evaluation_report.txt")
    ...
    print(f"\nIPW Baseline Comparison:")
    print(f"  IPW Random targeting: {random_uplift:.4f} ({random_uplift*100:.2f}%)")
    print(f"  IPW All-users targeting: {all_users_uplift:.4f} ({all_users_uplift*100:.2f}%)")
    
    if len(segment_metrics) >= 2:
        high_uplift = max(segment_metrics, key=lambda x: x['uplift_absolute'])
        low_uplift = min(segment_metrics, key=lambda x: x['uplift_absolute'])
        print(f"\nIPW Segment Differences:")
        print(f"  High-uplift cohort: {high_uplift['uplift_absolute']:.4f} ({high_uplift['uplift_absolute']*100:.2f}%)")
        print(f"  Low-uplift cohort: {low_uplift['uplift_absolute']:.4f} ({low_uplift['uplift_absolute']*100:.2f}%)")
