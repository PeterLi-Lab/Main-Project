...
def deep_feature_analysis():
    """Deep analysis of features that may cause issues"""
    ...
    if 'ai_interest_x_treatment' in df.columns:
        ...
        print(f"  Is interaction feature calculation correct: {is_correct}")
    ...
    for col in ai_related_features:
        ...
        # Check for obvious distribution differences
        mean_diff = treatment_group.mean() - control_group.mean()
        print(f"  Mean difference: {mean_diff:.4f}")
        
        if abs(mean_diff) > 0.1:
            print(f"  ⚠️  This feature shows obvious differences between treatment and control groups, potential data leakage")
    ...
    for col in future_features:
        ...
        # If treatment group feature values are significantly higher than control group, potential data leakage
        mean_diff = treatment_group.mean() - control_group.mean()
    ...
    if len(high_corr_pairs) > 0:
        issues.append(f"Found {len(high_corr_pairs)} highly correlated feature pairs, indicating multicollinearity")
    
    if len(complex_features) > 0:
        issues.append(f"Found {len(complex_features)} overly complex features")
    
    if len(outlier_features) > 0:
        issues.append(f"Found {len(outlier_features)} features with high outlier ratio")
    ...
    if issues:
        ...
        print("  4. Address outlier issues")
    else:
